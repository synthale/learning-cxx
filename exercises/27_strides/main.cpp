...
std::vector<udim> strides(std::vector<udim> const &shape) {
    ...
    //       使用逆向迭代器可能可以简化代码
    int size = 1;
    std::vector<udim>::reverse_iterator iter = strides.rbegin();
    std::vector<udim>::const_reverse_iterator shape_iter = shape.rbegin();
    for (; iter != strides.rend() && shape_iter != shape.rend(); iter++, shape_iter++) {
        *iter = size;
        size *= *shape_iter;
    }
    return strides;
